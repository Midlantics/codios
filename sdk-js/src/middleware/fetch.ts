...
export function withCodios(options: FetchCodiosOptions, handler: FetchHandler) {
  return async (req: Request): Promise<Response> => {
    ...
    if (!encoded) {
      return Response.json(
        { error: "missing_contract", message: "No Codios contract provided. Obtain one at codios.midlantics.com" },
        { status: 401 },
      );
    }
    ...
    if (options.gatewayUrl) {
      try {
        ...
        if (!nonceRes.ok) {
          ...
          return Response.json(
            { error: "contract_invalid", reason: body.reason ?? "nonce_rejected" },
            { status: 403 },
          );
        }
      } catch {
